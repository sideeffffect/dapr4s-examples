...
class Example11JobsTest extends E2ESuite:
  ...
  test("scheduled job fires back to its JobRoute, which persists the payload") {
    // Ask the app (via its `schedule` service method) to schedule demo-job ~2s out.
    ...
    while found.isEmpty && System.currentTimeMillis() < deadline do
      val (sc, body) = DaprHttp.get(infra.daprHttpPort, "/v1.0/state/statestore/job-result-demo-job")

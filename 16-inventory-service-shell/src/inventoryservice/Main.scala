...
// ── Impure shell (Scala.js) — twin of the JVM 13-inventory-service shell ─────
...
@main def inventoryServer(): Unit =
  val config = daprConfigFromEnv(defaultAppPort = 8080)
  println(s"=== 16 order-fulfillment (Scala.js): inventory-service on port ${config.appServer.port} ===")
  js.async {
    Dapr(config).serve:
      InventoryApp()
  }: Unit
